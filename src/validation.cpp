...
#if defined(NDEBUG)
# error "SuperAlki cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "SuperAlki Signed Message:\n";
using namespace std;
// Internal stuff
namespace {
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SALK additional fees, %d delta bytes\n",
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams)) { 
        DbgMsg(" %d , %0x , %0x" ,nHeight ,block.nBits  , GetNextWorkRequired(pindexPrev, &block, consensusParams) );
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // SuperAlki: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
