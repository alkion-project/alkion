Alkion Core version 0.13.2 is now available from:
  ...
  <https://download.alkion.org/alkion-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/alkion-project/alkion/issues>
...
updates will be released anymore. Without security updates, using a alkion
...
This allows running alkiond without having to do any manual configuration.
...
`alkion.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /alkionrpc>
        ...
        # optional bypass alkiond rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< alkionrpc:<password>
    </Location>
...
Restarting alkiond will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in alkion.conf
...
For example:
    ...
    $ src/alkion-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `alkion-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `alkion-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of alkion they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of alkion being spent is exactly
...
- `alkion-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `alkion-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
